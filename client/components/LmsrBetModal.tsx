...
export function LmsrBetModal({
  ...
  const [selectedOptionId, setSelectedOptionId] = useState<string | null>(
    initialOptionId ||
      (market.marketType === "multi" ? null : market.options[0]?.id || "yes"),
  );
  ...
  const selectedOption = useMemo<MarketOption | undefined>(
    () =>
      market.options.find((option) => option.id === selectedOptionId) ||
      (market.marketType !== "multi" ? market.options[0] : undefined),
  ...
  const userHolding = useMemo(() => {
    const pos = user?.positions?.find(
      (entry) =>
        entry.marketId === market.id && entry.optionId === selectedOptionId,
    );
    if (pos) return pos;
    ...
    if (user?.holdings) {
      const oldHolding = user.holdings.find(
        (h) =>
          ...
          (market.marketType === "multi"
            ? `option-${h.teamIndex}` === selectedOptionId
            : true),
      );
      if (oldHolding) {
        const shares =
          selectedOptionId === "yes"
            ? oldHolding.yesShares
            : selectedOptionId === "no"
              ? oldHolding.noShares
              : 0;
        if (shares > 0)
          return { marketId: market.id, optionId: selectedOptionId!, shares };
      }
    }
    return undefined;
  }, [
  ...
  const userPnl = useMemo(() => {
    ...
    const marketTrades = user.tradeHistory.filter(
  ...
  const isTimeClosed = market.endDate
  ...
  const isMultiNoPick =
    market.marketType === "multi" && !initialOptionId && !selectedOptionId;
  const activeTeamName =
    market.marketType === "multi" ? selectedOption?.name : undefined;
  ...
  const handleTrade = async () => {
    if (isGuestUser) {
      toast({
        ...
        description: "Log in with your IITR account to place trades.",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleMax = () => {
    if (!user) return;
    setAmount(
      String(tradeType === "buy" ? user.balance : userHolding?.shares || 0),
    );
  };
  ...
  const chartData = useMemo(() => {
    ...
    if (history.length <= 1) {
      const startTimestamp =
        market.createdAt || new Date(Date.now() - 86400000).toISOString();
        ...
        {market.options.map((opt, idx) => (
          <div
            ...
            className={cn(
              ...
              selectedOptionId === opt.id && "opacity-100",
            )}
          >
            <span
  ...
  const tradeControls = (
    <div className="flex flex-col gap-5">
      ...
      <div
        className={cn(
          "grid gap-2",
          market.options.length > 2 ? "grid-cols-2" : "flex gap-2",
        )}
      >
        {market.options.map((opt, idx) => (
          <button
      ...
      <div className="space-y-3">
        <div className="flex justify-between items-center text-xs">
          <span className="text-muted-foreground font-medium uppercase tracking-wider">
            {tradeType === "buy" ? "Amount in ₹" : "Shares to Sell"}
          </span>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-muted-foreground">
                Expected {tradeType === "buy" ? "Shares" : "Payout"}
              </span>
              <span className="font-bold">
                {tradeType === "buy"
                  ? quote.expectedShares.toFixed(2)
                  : `₹${quote.netPayout.toFixed(2)}`}
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-muted-foreground">Average Price</span>
              <span className="font-bold">
                ₹{quote.averagePrice.toFixed(4)}
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-muted-foreground">Current Spot</span>
              <span className="font-bold">
                {quote.currentPrice.toFixed(2)}p
              </span>
            </div>
            {tradeType === "sell" && (
              <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button
            ...
            className={cn(
              ...
              tradeType === "buy" ? "bg-yes text-white" : "bg-no text-white",
            )}
          >
            {isSubmitting ? (
        ...
        <Button
          ...
          disabled={
            ...
            (tradeType === "buy"
              ...
              : numericAmount > (userHolding?.shares || 0) + 1e-6)
          }
          className={cn(
            "w-full h-12 text-base font-bold",
            tradeType === "buy" ? "bg-primary" : "bg-foreground",
          )}
        >
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground leading-relaxed">
              You can browse this market, watch the graph, and read comments,
              but trading is disabled for guests.
            </p>
      ...
      <Drawer open={isOpen} onOpenChange={onClose}>
        <DrawerContent className="max-h-[90vh]">
          ...
          <div className="p-4 flex flex-col gap-5 overflow-y-auto no-scrollbar">
            ...
            {isMarketClosed ? (
              <div className="p-8 text-center bg-muted/20 rounded-2xl border border-border">
                <div className="text-[10px] font-black uppercase text-muted-foreground mb-4">
                  Market Resolved
                </div>
                <div className="text-3xl font-black text-primary mb-2">
                  {market.resolvedOptionId
                    ? market.options.find(
                        (o) => o.id === market.resolvedOptionId,
                      )?.name
                    : "RESOLVED"}
                </div>
                {userPnl !== null && userPnl !== 0 && (
                  <div
                    className={cn(
                      "mt-4 pt-4 border-t border-border flex flex-col items-center",
                      userPnl > 0 ? "text-green-500" : "text-destructive",
                    )}
                  >
                    <span className="text-[10px] font-black uppercase tracking-widest opacity-80 mb-1">
                      {userPnl > 0 ? "Total Earnings" : "Total Loss"}
                    </span>
                    <span className="text-2xl font-black">
                      ₹{Math.abs(userPnl).toFixed(2)}
                    </span>
                  </div>
                )}
              </div>
          ...
          <div className="p-4 border-t border-border mt-auto">
            ...
            {(isMarketClosed || isGuestUser || isMultiNoPick) && (
              <Button
                variant="outline"
      ...
      >
        <div className="flex flex-row min-h-[500px] max-h-[85vh]">
          ...
          <div className="flex-[1.5] flex flex-col p-8 border-r border-border overflow-y-auto no-scrollbar">
            <DialogTitle className="text-2xl font-black mb-6 leading-tight">
              {modalTitle}
            </DialogTitle>
          ...
          <div className="w-[380px] flex flex-col p-8 bg-muted/10">
              ...
              {isMarketClosed ? (
                <div className="space-y-6 py-8 flex flex-col items-center text-center">
                  ...
                  <div className="w-full space-y-3">
                    <div className="flex justify-between text-xs font-bold uppercase py-2 border-b border-border/50">
                      <span>Volume</span>
                      <span>₹{market.volume.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between text-xs font-bold uppercase py-2 border-b border-border/50 text-yes">
                      <span>Payout</span>
                      <span>₹1.00 / Share</span>
                    </div>
                  </div>
                </div>
              ) : isGuestUser ? (
            ...
            <div className="mt-8">
              {!isMarketClosed && !isMultiNoPick && !isGuestUser ? (
                submitButton
              ) : (
                <Button
                  variant="outline"
