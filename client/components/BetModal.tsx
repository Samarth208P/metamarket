...
export function BetModal({
  ...
  const [internalTeamIdx, setInternalTeamIdx] = useState<number | null>(
    market.teamIndex ?? null,
  );
  const isMultiNoPick =
    market.marketType === "multi" &&
    market.teams &&
    market.teams.length > 0 &&
    internalTeamIdx === null;
  ...
  const activePool: MarketPool = activeTeam
    ? {
        yesPool: activeTeam.yesPool,
        noPool: activeTeam.noPool,
        totalLiquidity: activeTeam.yesPool + activeTeam.noPool,
      }
    : market.pool;
  ...
  const probableWinnings =
    isBuying && priceRatio > 0 ? numAmount / priceRatio : 0;
    ...
    try {
      const response = await fetch(`/mapi/markets/${market.id}/trade`, {
        ...
        body: JSON.stringify({
          ...
          amount: numAmount,
          teamIndex: internalTeamIdx ?? market.teamIndex,
        }),
      });
      ...
      if (!response.ok) {
        ...
        return toast({
          title: "Trade Failed",
          description: payload?.error || "Unable to place trade",
          variant: "destructive",
        });
      }
  ...
  const titleText = activeTeamName
    ? `${market.title} - ${activeTeamName}`
    : market.title;
    ...
    if (userHolding) {
      const winningShares =
        market.resolvedOutcome === "yes"
          ? userHolding.yesShares
          : userHolding.noShares;
      return winningShares;
    }
    const payoutTrade = user.tradeHistory?.find(
      (t) =>
        t.marketId === market.id &&
        t.tradeType === "payout" &&
        (internalTeamIdx === undefined ||
          t.marketTitle?.includes(`[${activeTeamName}]`)),
    );
                      ...
                      <div className="bg-card/95 backdrop-blur-sm border border-border p-3 rounded-xl shadow-xl text-xs min-w-[150px] space-y-2">
                        {market.marketType === "multi" && market.teams ? (
                          market.teams.map((t, idx) => {
                            const val = payload.find(
                              (p) => p.dataKey === `team_${idx}`,
                            )?.value as number;
                          ...
                          <>
                            <div className="flex justify-between items-center">
                              ...
                              <span className="font-black text-yes text-sm">
                                {(payload[0].value as number).toFixed(1)}p
                              </span>
                            </div>
                            <div className="flex justify-between items-center text-muted-foreground/50 italic text-[10px]">
                              <span>{teamBLabel}</span>
                              <span>
                                {(100 - (payload[0].value as number)).toFixed(
                                  1,
                                )}
                                p
                              </span>
        ...
        {market.marketType === "multi" && market.teams ? (
          market.teams.map((t, idx) => (
            <div
              key={idx}
              className={cn(
                "flex items-center gap-1.5 opacity-60 transition-opacity",
                internalTeamIdx === idx && "opacity-100",
              )}
            >
              <span
  ...
  const multiTeamSelector = market.teams ? (
    <div className="flex flex-col gap-4">
      {countdownDisplay}
      <h4 className="text-[10px] font-black uppercase text-muted-foreground tracking-[0.2em]">
    ...
    <div className="flex flex-col gap-5">
      ...
      >
        ...
        >
          <span
            className={cn(
              "text-sm font-black mb-1 uppercase tracking-wider",
              outcome === "yes" ? "text-yes" : "text-muted-foreground",
            )}
          >
            {market.marketType === "versus" ? market.shortA : "Yes"}
          </span>
          <span className="text-2xl font-black">
            {activeYesPrice.toFixed(0)}p
          </span>
        </button>
        <button
        ...
        >
          <span
            className={cn(
              "text-sm font-black mb-1 uppercase tracking-wider",
              outcome === "no" ? "text-no" : "text-muted-foreground",
            )}
          >
            {market.marketType === "versus" ? market.shortB : "No"}
          </span>
          <span className="text-2xl font-black">
            {activeNoPrice.toFixed(0)}p
          </span>
        </button>
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="relative">
          ...
          <Input
            ...
            className={cn(
              "pr-16 h-12 text-lg font-bold bg-muted/30 border-border",
              isBuying ? "pl-7" : "pl-3",
            )}
          />
        ...
        <div className="flex gap-2 pt-1">
          {[10, 50, 100, 500].map((amt) => (
            <button
              ...
              className="flex-1 py-1.5 rounded-md bg-muted/50 hover:bg-muted text-xs font-semibold transition-colors"
            >
              +{amt}
            </button>
          ))}
          ...
          {isBuying && (
            <div className="flex justify-between text-sm">
              ...
              <span className="font-bold text-green-500">
                ₹{probableWinnings.toFixed(2)} (
                {(((probableWinnings - numAmount) / numAmount) * 100).toFixed(
                  0,
                )}
                %)
              </span>
      ...
      {isConfirming ? (
        <div className="flex gap-2">
          ...
          <Button
            ...
            disabled={isSubmitting}
            className={cn(
              "flex-[2] h-12 text-base font-black uppercase tracking-widest relative overflow-hidden",
              isBuying
          ...
          >
            {isSubmitting ? (
              ...
              <span className="relative z-10">
                Confirm{" "}
                {market.marketType === "multi" && activeTeamName
                  ? `[${activeTeamName}] `
                  : ""}
                {outcome === "yes" ? teamALabel : teamBLabel}{" "}
                {isBuying ? "Buy" : "Sell"}
              </span>
            )}
          </Button>
          ...
          disabled={
            ...
            (!isBuying &&
              numAmount >
                ((outcome === "yes"
                  ? userHolding?.yesShares
                  : userHolding?.noShares) || 0) +
                  0.01)
          }
          className={cn(
            "w-full h-12 text-base font-bold transition-all active:scale-95",
            isBuying
        ...
        >
          {user
            ? `Review ${market.marketType === "multi" && activeTeamName ? `[${activeTeamName}] ` : ""}${outcome === "yes" ? teamALabel : teamBLabel} ${isBuying ? "Buy" : "Sell"}`
            : "Log In To Trade"}
        </Button>
      )}
    </div>
      ...
      <Drawer open={isOpen} onOpenChange={onClose}>
        <DrawerContent
          className={cn(
            ...
            isConfirming
              ? "ring-4 ring-green-500/50 shadow-[0_0_30px_rgba(34,197,94,0.3)] shadow-green-500/20"
              : "",
          )}
        >
          <DrawerHeader className="text-left border-b border-border pb-4 relative">
            <DrawerTitle className="text-lg leading-tight font-bold pr-8">
              {titleText}
            </DrawerTitle>
            <DrawerClose asChild>
              <button
          ...
          <div className="flex flex-col gap-5 p-4 overflow-y-auto max-h-[80vh] no-scrollbar">
            ...
            <div className="space-y-6">
              <div>
                <h4 className="text-[10px] font-black uppercase text-muted-foreground mb-2 tracking-[0.2em]">
                  About this market
                </h4>
              ...
              <div className="space-y-6 pt-4">
                <div className="p-6 rounded-2xl bg-muted/30 border border-border flex flex-col items-center text-center">
                  ...
                  {isResolved ? (
                    ...
                    <>
                      <div className="text-2xl font-black uppercase text-muted-foreground mb-1">
                        Trading Ended
                      </div>
                ...
                <div className="flex justify-between items-center py-2 border-b border-border/50 bg-green-500/5 px-2 -mx-2 rounded">
                  ...
                  <span className="text-base font-black text-green-600">
                    ₹1.00 <span className="text-xs font-medium">/ share</span>
                  </span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-border/50">
                  <span className="text-xs font-bold text-muted-foreground uppercase">
                  ...
                  <span className="text-base font-black">
                    {market.resolvedOutcome === "yes" ? "100" : "0"}p
                  </span>
                </div>
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent
        className={cn(
          ...
          isConfirming
            ? "ring-4 ring-green-500/50 shadow-[0_0_40px_rgba(34,197,94,0.4)] border-green-500/40"
            : "",
        )}
      >
        <div className="flex flex-row h-full max-h-[85vh] relative z-0">
          ...
          <div className="flex-[1.5] flex flex-col p-8 border-r border-border min-w-[500px]">
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar space-y-8">
              <div className="min-h-[300px]">{graphElement}</div>

              <div className="space-y-6">
                <div>
                  <h4 className="text-[10px] font-black uppercase text-muted-foreground mb-3 tracking-[0.2em]">
                    About this market
                  </h4>
          ...
          <div
            className={cn(
              "w-[380px] flex flex-col p-8",
              isResolved ? "bg-muted/5 border-l border-border" : "bg-muted/10",
            )}
          >
                ...
                <div className="space-y-8">
                  ...
                  <div className="pt-4">
                    <p className="text-xs text-center text-muted-foreground font-medium leading-relaxed">
                      This market has been resolved by the community and is no
                      longer open for trading.
                    </p>
                  </div>
                </div>
              ) : isMultiNoPick ? (
